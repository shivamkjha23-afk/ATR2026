...
function setupAdminPanel() {
  ...
  function renderUsers() {
    ...
    const filteredUsers = users.filter((user) => {
      if (userFilter.value === 'pending') return !user.approved;
      if (userFilter.value === 'approved') return user.approved;
      return true;
    });
  ...
  function applyInspectionExcelRows(rows) {
    ...
    rows.forEach((row) => {
      ...
      if (!normalized.unit_name && defaultUploadUnit.value) {
        normalized.unit_name = defaultUploadUnit.value;
      }
    ...
    message.textContent = `Inspection upload complete. Updated: ${updated}, Added: ${inserted}.`;
  }

  function applyUsersExcelRows(rows) {
    const existing = readJSON(STORAGE_KEYS.users);
    const byUsername = new Map(existing.map((item) => [String(item.username), item]));
    let updated = 0;
    let inserted = 0;
    ...
    rows.forEach((row) => {
      ...
      if (byUsername.has(username)) {
        Object.assign(byUsername.get(username), normalized);
        updated += 1;
      } else {
        existing.push(normalized);
        inserted += 1;
    ...
    saveJSON(STORAGE_KEYS.users, existing);
    message.textContent = `User upload complete. Updated: ${updated}, Added: ${inserted}.`;
    renderUsers();
  }
  ...
  function downloadTemplate() {
    ...
    if (uploadType === 'users') {
      const templateRow = [{ username: 'new.user', role: 'inspector', approved: false, request_date: '2026-01-20', approved_by: '' }];
  ...
  document.getElementById('bulkUploadBtn').addEventListener('click', processExcelFile);
